...
func fsMessageHandler(server *FSserver, resp []byte, bytes_read int, membership_server * Server) {
	...
	switch message.MessageType {
		case REPLICATE:
			fromPath := "ruiqip22@" + remove_port_from_hostname(message.Info_Hostname) + ":" + local_folder_path + message.LocalFilename
		...
		case RE_REPLICATE:
			fromPath := "ruiqip22@" + remove_port_from_hostname(message.Info_Hostname) + ":" + sdfs_folder_path + message.SdfsFilename
